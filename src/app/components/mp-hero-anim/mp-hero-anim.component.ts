...
export class MpHeroAnimComponent implements AfterViewInit {
  texts: string[] = ['Mike Ponton', 'a Tech Addict', 'a creative mind', 'a Game Developer', 'an aspiring web developer']; 
  ...
  animateText() {
    let textElement = this.el.nativeElement.querySelector('.hero-text') as HTMLElement; // Select the text element
    
    if (textElement) {
      this.typeText(textElement, this.texts[this.currentTextIndex], 'type', () => {
        setTimeout(() => {
  ...
  typeText(element: HTMLElement, text: string, action: Action, callback: () => void) {
